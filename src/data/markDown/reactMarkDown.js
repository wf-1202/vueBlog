export const componentHooks = {
  text:
    "## react组件写法 + Hooks\n" +
    "### 记录个例子，以后写的话作为参考。\n" +
    `      import { Form, Input, Button, Select, InputNumber, Modal, message, Spin } from 'antd';
                      ...
                      >
                          ...
                          <div className={'pay'}>
                              <Divider orientation="left">
                                  <div>
                                      <span className={'address-icon'}> <CheckOutlined/></span> <Tag
                                      className={'address-text'} color="orange">订单地址保存成功</Tag>
                                  </div>
                              </Divider>
                              <div className={'orderTitle'}><span>￥</span><span>{number_format(store.orderAmount, 2)} </span>
                              </div>
                              <div className={'barcode'}>
                                  <BarcodeOutlined/>
                              </div>
                              <Form.Item name="wxPayAuthCode" rules={[{ required: true, message: '付款码不能为空' }]}>
                                  <Input
                                      id={"wxPayAuthCode"}
                              ...
                              <div className={'remark'}>说明：订单支付成功后，处方将推送到调剂中心</div>
