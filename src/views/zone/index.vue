...
export default {
  ...
  data() {
    return {
      ...
      routes: [
        ...
        {
          title: "其它文章",
          route: `/zone/type?type=${this.$route.query.type}`,
        },
        // {
        //   title: '时间轴',
        //   route: '/zone/timeline'
  ...
  methods: {
    handleScroll(e) {
      // 处理滚动事件 （动态处理author组件距离内容顶部位置）
      if (this.contentHeight < this.authorHeight) {
        // 当内容高度小于author高度，不做任何处理
        return;
      }
      ...
      if (
        ...
        scrollTop - this.lastScroll < this.authorHeight
      ) {
